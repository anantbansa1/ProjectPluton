...
import { query, collection, doc, getDoc, getDocs, where, serverTimestamp, addDoc, setDoc } from "firebase/firestore";
  ...
  async function handlepost() {
    if (option === 'text') {
      if (text === '') {
        ...
        const payload = { text: text, timestamp: serverTimestamp(), visibility: visibility, clubname: clubName, imageurl: '', tag: tags[selected] };
  ...
  async function checkClub() {
    if (user) {
      ...
      try {
        ...
        const currentclub = await getDocs(query(collection(db, 'clubs'), where('name', '==', clubName)));
        if (!currentclub) {
          navigate('/pagenotfound')
          return;
        }
        const currentuser = await getDocs(query(collection(db, 'user'), where('email', '==', email)));
  ...
  const handleSubmit = () => {
    ...
    if (canvas) {
      canvas.toBlob((blob) => {
        ...
        const n = (file.lastModifiedDate).toString();
        ...
        uploadBytes(storageRef, file)
          .then((snapshot) => {
            ...
            getDownloadURL(storageRef)
              .then((u) => {
                const payload = { text: text, timestamp: serverTimestamp(), visibility: visibility, clubname: clubName, imageurl: u, tag: tags[selected] };
        ...
        <div className="flex flex-col w-[50vw] max-sm:w-[75vw] max-lg:w-[65vw] mx-auto">
                    ...
                    <div className="shadow-lg mb-5 shadow-black">
                      <div className="grid grid-rows-1 grid-cols-1 " >
                        {CoverImage ? (
                          ...
                          <div onClick={handleClickOpenCover} className="text-slate-400 text-lg  hover:bg-[#0e0b1a] cursor-pointer  w-[80vw] h-[80vw] md:w-[50vw] md:h-[50vw] lg:w-[30vw] lg:h-[30vw] rounded-2xl row-start-1 col-start-1 bg-[#130f22] flex justify-center items-center">
          ...
          {option === "poll" && (
            ...
            <div className="flex flex-col ">
              <div className=" bg-none  max-md:text-xl  flex items-center  text-3xl justify-between">
              </div>
          ...
          <div className="flex space-x-5 w-[100%] my-10 justify-between">
            <div className="">
              ...
              <div className="">
                ...
                >
                  <MenuItem onClick={() => handleCloseVisibility('Public')} sx={{
                    bgcolor: visibility === 'Public' ? '#0e0c1b' : 'inherit',
                  }}>
                    {" "}
                    Public
                  </MenuItem>
                  <MenuItem onClick={() => handleCloseVisibility('Members')} sx={{
