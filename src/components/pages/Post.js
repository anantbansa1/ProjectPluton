  ...
  return (
    <div>
      <div className="ml-[20vw] max-md:ml-[18vw] flex  my-10  max-sm:mr-0">
        <div className={`flex items-center flex-col mx-auto ${props.image !== "" ? "w-fit h-fit" : "w-[100%] md:w-[45vw]   h-fit"} max-sm:pb-5 bg-[#130f22] shadow-xl rounded-md max-md:py-4 py-8 max-sm:px-0 px-10  shadow-black text-white`}>
          <div className="flex justify-between font-semibold items-center self-start w-full">
            <Link to={`/club/${props.name}`} className="flex items-center max-sm:space-x-2 space-x-5">
              <img
                src={props.ClubImage}
                alt=""
          ...
          <div className={`flex  flex-col ${props.image !== '' ? 'items-center' : 'items-start'} space-y-5 w-full`}>
            {props.image !== "" && (

              <div className="mt-5 sm:mx-auto  w-[40vw]  max-md:w-[100%] ">
                <img src={props.image} className="object-cover h-[100%] w-[100%] max-md:w-[100%] max-md:h-[100%] rounded-md" alt="" />
                <div className="text-lg my-5 max-sm:my-2 max-md:text-sm text-[#dddbdb] ">
                  {props.text !== "" && (
                    <span className="font-bold text-white">{props.name}{" "}</span>)} <span className="text-justify"> {props.text}</span>
                </div>
              </div>
